...
To roll back, set `FAULTLANE_ALERTS_ENABLED=false` on all roles and restore the previous application build. The migration is additive. Existing integrations, encrypted configuration, rules, condition state, and delivery history remain available for a corrected build. Do not remove the integration key while queued or historical configuration still needs to be read.
