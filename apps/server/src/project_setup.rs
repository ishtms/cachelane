...
impl ServerState {
    pub(crate) async fn from_environment(host: &str) -> Result<Self, StartupError> {
...
pub(crate) fn router(role: &'static str, state: ServerState) -> Router {
    ...
    match role {
        "api" => health
            ...
            .with_state(state),
        "ingest" => health
            .route("/u/{key}", post(resolve_ingest_key))
            .with_state(state),
...
async fn create_setup(
    ...
    let created = state
        .store
        .bootstrap(BOOTSTRAP_SUBJECT, &request, &key)
        .await?;
...
async fn rotate_ingest_key(
    ...
    let created = state
        .store
        .rotate_key(BOOTSTRAP_SUBJECT, &project_id, &key)
        .await?;
    let response = created_response(&state.ingest_base_url, created, key);
...
async fn resolve_ingest_key(
    ...
    let digest = hash(&key);
    if state.store.resolve_key(&digest).await?.is_none() {
        return Err(ApiError::NotFound);
    }

    Err(ApiError::IngestNotReady)
}
    ...
    fn into_response(self) -> Response {
        let (status, code, message) = match self {
            ...
            Self::IngestNotReady => (
                ...
                "ingest_not_ready",
                "durable crash ingestion is not available",
            ),
            Self::Internal => (
...
impl ProjectStore {
    async fn bootstrap(
        ...
        match self {
            Self::Postgres(pool) => postgres_bootstrap(pool, subject, request, key).await,
            #[cfg(test)]
            Self::Memory(store) => memory_bootstrap(store, subject, request, key),
        }
    }
    ...
    async fn rotate_key(
        ...
        match self {
            Self::Postgres(pool) => postgres_rotate_key(pool, subject, project_id, key).await,
            #[cfg(test)]
            Self::Memory(store) => memory_rotate_key(store, subject, project_id, key),
        }
    }
...
async fn postgres_bootstrap(
    ...
    .await
    .map_err(map_sqlx_error)?;
    let stored_key = insert_key(&mut transaction, &organization_id, &project_id, key).await?;
...
async fn insert_key(
    ...
    let row = sqlx::query(
        "INSERT INTO project_ingest_keys (organization_id, project_id, secret_hash, display_suffix) VALUES ($1::uuid, $2::uuid, $3, $4) RETURNING id::text AS id, to_char(created_at AT TIME ZONE 'UTC', 'YYYY-MM-DD\"T\"HH24:MI:SS.US\"Z\"') AS created_at",
    )
    .bind(organization_id)
    .bind(project_id)
    .bind(&key.digest)
...
async fn postgres_keys(
    ...
    let rows = sqlx::query(
        "SELECT k.id::text AS id, k.display_suffix, to_char(k.created_at AT TIME ZONE 'UTC', 'YYYY-MM-DD\"T\"HH24:MI:SS.US\"Z\"') AS created_at, CASE WHEN k.revoked_at IS NULL THEN NULL ELSE to_char(k.revoked_at AT TIME ZONE 'UTC', 'YYYY-MM-DD\"T\"HH24:MI:SS.US\"Z\"') END AS revoked_at FROM project_ingest_keys k JOIN projects p ON p.id = k.project_id AND p.organization_id = k.organization_id JOIN organization_memberships m ON m.organization_id = p.organization_id AND m.role = 'owner' JOIN users u ON u.id = m.user_id WHERE u.bootstrap_subject = $1 AND p.id::text = $2 ORDER BY k.created_at, k.id",
    )
    .bind(subject)
    .bind(project_id)
    .fetch_all(pool)
...
async fn postgres_resolve_key(
    ...
    let row = sqlx::query(
        "SELECT organization_id::text AS organization_id, project_id::text AS project_id FROM project_ingest_keys WHERE secret_hash = $1 AND revoked_at IS NULL",
    )
    .bind(digest)
    .fetch_optional(pool)
    .await
    ...
    Ok(row.map(|row| KeyScope {
        _organization_id: row.get("organization_id"),
        _project_id: row.get("project_id"),
    }))
}
...
fn memory_resolve_key(
    ...
    Ok(setup
        .keys
        .iter()
        .any(|key| !key.revoked && key.digest == digest)
        .then(|| KeyScope {
            _organization_id: setup.view.organization.id.clone(),
            _project_id: setup.view.project.id.clone(),
        }))
}
...
mod tests {
    ...
    use super::{
        BootstrapAuthorization, ServerState, StartupError, hash, migrate, router, valid_name,
        validate_ingest_base_url,
    };
    ...
    fn assert_isolated_database(database_url: &str) {
        let database_name = database_url
            ...
            .unwrap_or_default();
        assert!(database_name == "cachelane_295" || database_name.starts_with("cachelane_295_"));
    }
